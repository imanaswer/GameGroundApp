/** camps endpoints (§3.3). Uniform with the other registerable entities — one shape. */
...
export function list(params: { q?: string } = {}): Promise<RegisterableSummary[]> {
  ...
  return api.get<RegisterableSummary[]>(`/camps${s}`);
}

export function detail(id: string): Promise<RegisterableDetail> {
  return api.get<RegisterableDetail>(`/camps/${id}`);
}
