/** camps endpoints (§3.3). Reconciled through the shared registerable mapper. */
...
export async function list(params: { q?: string } = {}): Promise<RegisterableSummary[]> {
  ...
  const rows = await api.get<RawRegisterable[]>(`/camps${s}`);
  return rows.map((r) => toSummary(r, "camp"));
}

export async function detail(id: string): Promise<RegisterableDetail> {
  return toDetail(await api.get<RawRegisterable>(`/camps/${id}`), "camp");
}

/** Free registration (camps with price 0). Paid ones go through checkout and 402 here. */
export function register(id: string, fields: Record<string, unknown>) {
  return registerFree("camps", id, fields);
}

/** Cancel this registration (90-minute pre-start cutoff, server-enforced). */
export function cancel(id: string) {
  return cancelReg("camps", id);
}
